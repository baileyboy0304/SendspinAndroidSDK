...
import com.mph070770.sendspinandroid.audio.AudioJitterBuffer
import com.mph070770.sendspinandroid.audio.OpusDecoder
import com.mph070770.sendspinandroid.audio.PcmAudioOutput
import com.mph070770.sendspinandroid.service.ServiceUiState
import com.mph070770.sendspinandroid.sync.SendspinTimeFilter
...
internal class SendspinPcmClient(
    private val wsUrl: String,
    private val clientId: String,
    private val clientName: String,
    private val onUiUpdate: ((ServiceUiState) -> ServiceUiState) -> Unit,
    ...
    private val clock = SendspinTimeFilter()
    ...
    suspend fun connect() {
        Log.d(tag, "===== PCMCLIENT CONNECT CALLED =====")
        Log.d(tag, "WebSocket URL: $wsUrl")
        Log.d(tag, "Client ID: $clientId, Name: $clientName")

        val req = Request.Builder().url(wsUrl).build()
        Log.d(tag, "Built WebSocket request")

        onUiUpdate { it.copy(status = "connecting...", connected = false) }
        Log.d(tag, "Set UI state to connecting")

        Log.d(tag, "Creating WebSocket (async)...")
        ws = okHttp.newWebSocket(req, object : WebSocketListener() {
            ...
            override fun onMessage(webSocket: WebSocket, text: String) {
                // Reduced logging - only log important messages
                val obj = try { org.json.JSONObject(text) } catch (e: Exception) { null }
                val type = obj?.optString("type", "")
                if (type in listOf("server/hello", "stream/start", "stream/end", "group/update")) {
            ...
            override fun onMessage(webSocket: WebSocket, bytes: ByteString) {
                // Binary messages logged only in debug builds
        ...
        Log.d(tag, "WebSocket creation initiated")
        Log.d(tag, "===== PCMCLIENT CONNECT RETURNING =====")
    }
    ...
    private fun getCurrentAndroidVolume(): Int {
    ...
    private fun sendClientStateSynchronized(volume: Int? = null, muted: Boolean = false) {
        val actualVolume = volume ?: getCurrentAndroidVolume()
        val player = JSONObject().put("state", "synchronized").put("volume", actualVolume).put("muted", muted)
        sendJson("client/state", JSONObject().put("player", player))
    }
    ...
    private fun startStatsLoop() {
        ...
        statsJob = scope.launch {
            while (isActive && isConnected.get()) {
                ...
                delay(10000L)  // Every 10 seconds instead of 3
            }
        }
    }
        ...
        playoutJob = scope.launch {
            ...
            while (isActive && isConnected.get()) {
                ...
                if (!output.isStarted()) {
                    ...
                    // Calculate effective buffer ahead considering play_at timestamp
                    // This prevents starting too early when joining mid-track
                    val effectiveBufferAheadMs = if (playAtServerUs != Long.MIN_VALUE && snap2.headServerUs != null) {
                        val nowServerUs = nowUs() + offUs
                        val effectiveHeadUs = maxOf(snap2.headServerUs!!, playAtServerUs)
                        ((effectiveHeadUs - nowServerUs) / 1000L)
                    } else {
                        snap2.bufferAheadMs
                    }

                    // IMPORTANT: Wait for clock convergence before starting playback
                    // This prevents massive drops during Kalman filter cold start
                    val clockConverged = clock.hasConverged()

                    val canStart =
                        clockConverged &&  // NEW: Wait for time sync to converge
                    ...
                    if (canStart) {
                        ...
                        Log.i(tag, "START_DECISION: Starting playback - queued=${snap2.queuedChunks} bufferAhead=${snap2.bufferAheadMs}ms effective=${effectiveBufferAheadMs}ms play_at=${if (playAtServerUs != Long.MIN_VALUE) "set" else "unset"} clockMeasurements=${clock.getMeasurementCount()} clockError=${clock.estimatedErrorUs()}us")
                        Log.i(tag, "Audio output started sr=$sampleRate ch=$channels bd=$bitDepth codec=$codec (buffered=${snap2.bufferAheadMs}ms effective=${effectiveBufferAheadMs}ms)")
                    } else {
                        // Log why we're waiting (occasionally to avoid spam)
                        if (snap2.queuedChunks > 0 && snap2.queuedChunks % 50 == 0) {
                            Log.d(tag, "START_WAIT: clockConverged=$clockConverged measurements=${clock.getMeasurementCount()} error=${clock.estimatedErrorUs()}us queued=${snap2.queuedChunks} bufferAhead=${snap2.bufferAheadMs}ms effective=${effectiveBufferAheadMs}ms")
                        }
                        delay(10)
                        continue
                    }
                ...
                // Sync debug: Log actual timing relationship every ~3 seconds (1 in 100 chunks at 20ms/chunk)
                if (kotlin.random.Random.nextInt(100) == 0) {
                    val nowServerUs = nowUs() + offUs
                    val timingDeltaUs = effectiveServerTsUs - nowServerUs
                    val timingDeltaMs = timingDeltaUs / 1000L
                    Log.i(tag, "SYNC_TIMING: chunkTs=$effectiveServerTsUs serverNow=$nowServerUs delta=${timingDeltaMs}ms " +
                            "(playing ${if (timingDeltaMs > 0) "${timingDeltaMs}ms EARLY" else "${-timingDeltaMs}ms LATE"}) " +
        ...
        try {
            ...
            when (type) {
                "server/hello" -> {
                    ...
                    sendClientStateSynchronized(volume = getCurrentAndroidVolume(), muted = false)
                }
